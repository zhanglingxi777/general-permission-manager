...
public class TokenInterceptor implements HandlerInterceptor {
    ...
    public static final List<String> WITHE_LIST = Arrays.asList("/sys/user/vcImage", "/sys/user/login", "/common/file/.*");

    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        String uri = request.getRequestURI();
        // 在白名单中的请求直接放行
        if (isMatchWhiteList(uri)) {
            return true;
        } else {
            boolean pass = true;
     ...
     * @return 是否在白名单中
     */
    private boolean isMatchWhiteList(String uri) {
        for (String withe : WITHE_LIST) {
            if (uri.matches(withe)) {
                return true;
            }
        }
        return false;
